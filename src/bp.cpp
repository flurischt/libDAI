...
namespace dai {
    ...
    void BP::construct() {
        ...
        _indices.push_back({0, 0, 1, 1});
        DAI_DEBASSERT(_indices.size() - 1 == INDEX_0011);
        _indices.push_back({0, 1, 0, 1});
        DAI_DEBASSERT(_indices.size() - 1 == INDEX_0101);
        _indices.push_back({0, 1});
        DAI_DEBASSERT(_indices.size() - 1 == INDEX_01);
...
// our first pattern 0011. Then we want to add (1->0, 2->1) and (3->0, 4->1) which gives us our second pattern 0101.
    ...
    void BP::marginalizeProductOntoMessage(__m256d &avx_prod, size_t i, size_t _I, size_t prodsize) {
        __m256d values = _mm256_hadd_pd(avx_prod, _mm256_permute2f128_pd(avx_prod, avx_prod,
                                                                         1)); //a0 + a1, a2 + a3, a2 + a3, a0 + a1
    ...
    Real BP::run() {
        ...
        if (props.verbose >= 1) {
            if (maxDiff > props.tol) {
                if (props.verbose == 1)
                    cerr << endl;
                cerr << name() << "::run:  WARNING: not converged after " << _iters << " passes (" << toc() - tic <<
